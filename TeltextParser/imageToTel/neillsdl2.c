...
void Decoder(Instruction *instruct, unsigned char code[]){
    ...
    /*The code from 0x81 to 0x87 are about changing colour,
    the colour could be either background's colour or character's colour.*/
    /*There are to patterns. One is "0x8X 0x9d", the other is "0x8x NO-ox9d" 
    The first one is used to change background's colour.'*/
    ...
    /*check invalid code*/
        ...
        instruct->data = SPACE;
        /*jjjjjjjj*/
        /*This is ambiguous. I just make an assumption that 
        "a0 9d" will change the background's colour to white 
        if(IsForBackColour(code, instruct->counter)){
            instruct->nextbackcolour = white;
        }*/
        instruct->ishold = FALSE;
    }
